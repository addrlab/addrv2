 ...
 * - Private keys never leave this worker except through postMessage to main thread
 * 
 * Ed25519 implementation based on tweetnacl (Public Domain / Unlicense)
 * https://github.com/dchest/tweetnacl-js
 */

// Type definitions (inline to avoid import issues in worker)
...
// =============================================================================
// Ed25519 Implementation (from tweetnacl, Public Domain)
// =============================================================================
...
function crypto_verify_32(x: Uint8Array, xi: number, y: Uint8Array, yi: number): number {
  let d = 0;
  for (let i = 0; i < 32; i++) d |= x[xi + i] ^ y[yi + i];
  return (1 & ((d - 1) >>> 8)) - 1;
}
...
function crypto_hashblocks_hl(hh: Int32Array, hl: Int32Array, m: Uint8Array, n: number): number {
  const wh = new Int32Array(16), wl = new Int32Array(16);
  let bh0: number, bh1: number, bh2: number, bh3: number, bh4: number, bh5: number, bh6: number, bh7: number;
  let bl0: number, bl1: number, bl2: number, bl3: number, bl4: number, bl5: number, bl6: number, bl7: number;
  let th: number, tl: number, h: number, l: number, a: number, b: number, c: number, d: number;
  ...
  while (n >= 128) {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      ah1 = bh0; ah2 = bh1; ah3 = bh2; ah4 = bh3; ah5 = bh4; ah6 = bh5; ah7 = bh6; ah0 = bh7;
      al1 = bl0; al2 = bl1; al3 = bl2; al4 = bl3; al5 = bl4; al6 = bl5; al7 = bl6; al0 = bl7;
...
function crypto_hash(out: Uint8Array, m: Uint8Array, n: number): number {
  const hh = new Int32Array(8), hl = new Int32Array(8);
 ...
 */
function crypto_sign_keypair_seed(pk: Uint8Array, sk: Uint8Array, seed: Uint8Array): number {
 ...
 */
function generateEd25519Keypair(): { publicKey: Uint8Array; secretKey: Uint8Array } {
...
function matchesPattern(publicKey: string, prefix: string, suffix: string, caseSensitive: boolean): boolean {
  ...
  return (!prefix || keyToCheck.startsWith(prefixToCheck)) && (!suffix || keyToCheck.endsWith(suffixToCheck));
}
...
async function generateVanityAddress(config: GeneratorConfig): Promise<void> {
  ...
  while (isRunning) {
    ...
    if (matchesPattern(publicKeyBase58, prefix, suffix, caseSensitive)) {
      ...
      // Full 64-byte secret key encoded as Base58 - required for Phantom import
